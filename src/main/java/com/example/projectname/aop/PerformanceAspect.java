...
@Component
@Slf4j
public class PerformanceAspect extends com.example.projectname.aop.Aspect {
    // 특정 어노테이션을 대상으로 지정
    @Pointcut("@annotation(com.example.projectname.annotation.RuningTime)")
    private void enableRunningTime() {}

    // 기본 패키지의 모든 메소드를 대상으로 지정
    @Pointcut("execution(* com.example.projectname..*.*(..))")
    private void cut() {}

    // 실행 시점 설정: 두 조건을 모두 만족하는 대상 전후로 부가 기능을 삽입
